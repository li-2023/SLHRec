...
class KnowledgeGraph(object):
  def __init__(self, facts, predicates, dataset, cmd_args):
    self.dataset = dataset
    self.graph, self.edge_type2idx, \
        self.ent2idx, self.idx2ent, self.rel2idx, self.idx2rel, \
        self.node2idx, self.idx2node = gen_graph(facts, predicates, dataset, cmd_args)
...
def gen_index(facts, predicates, dataset, cmd_args):
  rel2idx = dict()
  idx_rel = 0
  ...
  file_path = '../data/' + cmd_args.dataset + '/' 


  if os.path.exists(file_path + 'CF_data/final_graph.txt'):
    print("final_graph file exist")
  else:
    print("construct final_graph file")
    final_graph_file = open(file_path + 'CF_data/final_graph.txt', 'w') 

    for line in open(file_path + 'Fact_data/graph.txt', encoding='utf-8').readlines():
      array = line.strip().split('\t')
      link = str(int( ent2idx[array[0]] )) + "\t" + str(int( rel2idx[array[1]] )) + "\t" + str(int( ent2idx[array[2]] ))
      final_graph_file.write(link+"\n")

  if os.path.exists(file_path + 'CF_data/final_train.txt'):
    print("final_train file exist")
  else:
    print("construct final_train file")
    final_graph_file = open(file_path + 'CF_data/final_train.txt', 'w') 

    for line in open(file_path + 'Fact_data/train.txt', encoding='utf-8').readlines():
      array = line.strip().split('\t')
      link = str(int( ent2idx[array[0]] )) + "\t" + str(int( ent2idx[array[2]] )) + "\t" + str(1)
      final_graph_file.write(link+"\n")

  if os.path.exists(file_path + 'CF_data/final_test.txt'):
    print("final_test file exist")
  else:
    print("construct final_test file")
    final_graph_file = open(file_path + 'CF_data/final_test.txt', 'w') 

    for line in open(file_path + 'Fact_data/test.txt', encoding='utf-8').readlines():
      array = line.strip().split('\t')
      link = str(int( ent2idx[array[0]] )) + "\t" + str(int( ent2idx[array[2]] )) + "\t" + str(1)
      final_graph_file.write(link+"\n")

  if os.path.exists(file_path + 'CF_data/final_valid.txt'):
    print("final_valid file exist")
  else:
    print("construct final_valid file")
    final_graph_file = open(file_path + 'CF_data/final_valid.txt', 'w') 

    for line in open(file_path + 'Fact_data/valid.txt', encoding='utf-8').readlines():
      array = line.strip().split('\t')
      link = str(int( ent2idx[array[0]] )) + "\t" + str(int( ent2idx[array[2]] )) + "\t" + str(1)
      final_graph_file.write(link+"\n")
...
def gen_graph(facts, predicates, dataset, cmd_args):


  g = nx.Graph()
  ent2idx, idx2ent, rel2idx, idx2rel, node2idx, idx2node = gen_index(facts, predicates, dataset, cmd_args)
